...
The role goes through a "prepare" stage where it's setting up Kubernetes requirements for the OS
...
---
  ...
  1) Testing Flannel CNI is working properly (eg - routing pod network traffic across nodes), you can make use of the "ping pods". You should be able to ping across worker nodes!




  3) Testing MetalLB load-balancer is working as expected, you can access the IP for the "Demo Webapp" from your browser and watch the IP's on the page change as MetalLB balances the traffic to the various containers running on different nodes.
  4) HeadLamp is running also. It is a Web Interface for Managing the Kubernetes Cluster and should have a Kubernetes Service using MetalLB to expose the app to external LAN. Hint: kubectl get svc -o wide --all-namespaces |grep -i headlamp
